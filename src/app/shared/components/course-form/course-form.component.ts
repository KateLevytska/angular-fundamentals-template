...
import { AuthorsResponce, Author, AuthorsResponceAll, CourseResponce, Course } from '../../../interfaces';
...
export class CourseFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.buildForm();
    this.CoursesStoreService.getAllAuthors().pipe(take(1)).subscribe((e: AuthorsResponceAll) => {
      this.allAuthors = e.result
    });
    if (this.id) {

      forkJoin({
        currentCourse: this.CoursesStoreService.getCourse(this.id).pipe(map(r => r.result)),
  ...
  public createAuthor(): void {
    ...
    this.CoursesStoreService.createAuthor(payload).pipe(take(1)).subscribe((result: AuthorsResponce) => {
